...
--  begin read only
   ...
   procedure Test_Get_Channel_Arrays_Str_afbd18 (Gnattest_T : in out Test) renames Test_Get_Channel_Arrays_Str;
--  id:2.2/afbd186c8468829f/Get_Channel_Arrays_Str/1/0/
...
--  end read only
   ...
   begin
      ...
      Tmpl := Mutools.Templates.Create
        (Content => Get_Channel_Arrays_Str
           (Policy    => Policy,
            Comp_Name => "vt"));
      Mutools.Templates.Write (Template => Tmpl,
                               Filename => "obj/" & Fn);
      ...
      Assert (Condition => Get_Channel_Arrays_Str
              (Policy    => Policy,
               Comp_Name => "no_res") = "",
              Message   => "Content mismatch (2)");
--  begin read only
