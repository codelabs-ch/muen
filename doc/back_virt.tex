...
to a so called \emph{trap}\index{trap} from the guest code into the VMM. The
...
into host-state, guest-state, read-only data and control fields. On VM entry
for example, the state of the host is saved into the subject VMCS and restored
