...
is
   --D @Section Id => validation-knl, Label => Kernel, Parent => validation, Priority => 0
   --D @Text Section => validation-knl, Priority => 0
   --D The following kernel-specific checks are performed on the policy.
   --D @UL Id => validators_knl, Section => validation-knl, Priority => 0

   --D @Item List => validators_knl, Priority => 0
