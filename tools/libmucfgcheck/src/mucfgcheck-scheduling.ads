...
is
   --D @Section Id => validation-sched, Label => Scheduling, Parent => validation, Priority => 0
   --D @Text Section => validation-sched, Priority => 0
   --D The following checks are performed to verify the correctness of the
   --D scheduling configuration in the system policy.
   --D @UL Id => validators_sched, Section => validation-sched, Priority => 0

   --D @Item List => validators_sched, Priority => 0
