--  This package has been generated automatically by GNATtest.
--  Do not edit any part of it, see GNATtest documentation for more details.

--  begin read only
with Gnattest_Generated;

package Stackcheck.Types.Test_Data.Tests is

   type Test is new GNATtest_Generated.GNATtest_Standard.Stackcheck.Types.Test_Data.Test
   with null record;

   procedure Test_Create_649a54 (Gnattest_T : in out Test);
   --  stackcheck-types.ads:32:4:Create

   procedure Test_Get_Name_1246db (Gnattest_T : in out Test);
   --  stackcheck-types.ads:38:4:Get_Name

   procedure Test_Get_Stack_Usage_ae398b (Gnattest_T : in out Test);
   --  stackcheck-types.ads:41:4:Get_Stack_Usage

   procedure Test_Get_Max_Stack_Usage_1f4ad5 (Gnattest_T : in out Test);
   --  stackcheck-types.ads:45:4:Get_Max_Stack_Usage

   procedure Test_Set_Max_Stack_Usage_e177c6 (Gnattest_T : in out Test);
   --  stackcheck-types.ads:48:4:Set_Max_Stack_Usage

   procedure Test_Add_Call_a1b7c6 (Gnattest_T : in out Test);
   --  stackcheck-types.ads:53:4:Add_Call

   procedure Test_Get_Call_Count_14636b (Gnattest_T : in out Test);
   --  stackcheck-types.ads:58:4:Get_Call_Count

   procedure Test_Iterate_Calls_3d8be5 (Gnattest_T : in out Test);
   --  stackcheck-types.ads:62:4:Iterate_Calls

   procedure Test_Is_Active_3f2f51 (Gnattest_T : in out Test);
   --  stackcheck-types.ads:67:4:Is_Active

   procedure Test_Set_Active_31acca (Gnattest_T : in out Test);
   --  stackcheck-types.ads:70:4:Set_Active

   procedure Test_Is_Done_e074a1 (Gnattest_T : in out Test);
   --  stackcheck-types.ads:75:4:Is_Done

   procedure Test_Set_Done_969bfa (Gnattest_T : in out Test);
   --  stackcheck-types.ads:78:4:Set_Done

   procedure Test_Add_Node_e8151f (Gnattest_T : in out Test);
   --  stackcheck-types.ads:87:4:Add_Node

   procedure Test_Update_Node_6c7c3f (Gnattest_T : in out Test);
   --  stackcheck-types.ads:94:4:Update_Node

   procedure Test_Iterate_a24a0d (Gnattest_T : in out Test);
   --  stackcheck-types.ads:101:4:Iterate

   procedure Test_Add_Call_32d494 (Gnattest_T : in out Test);
   --  stackcheck-types.ads:107:4:Add_Call

   procedure Test_Get_Max_Stack_Usage_4e2158 (Gnattest_T : in out Test);
   --  stackcheck-types.ads:115:4:Get_Max_Stack_Usage

   procedure Test_Calculate_Stack_Usage_ebf2ac (Gnattest_T : in out Test);
   --  stackcheck-types.ads:122:4:Calculate_Stack_Usage

   procedure Test_Equal_Name_85b0c9 (Gnattest_T : in out Test);
   --  stackcheck-types.ads:153:4:Equal_Name

end Stackcheck.Types.Test_Data.Tests;
--  end read only
