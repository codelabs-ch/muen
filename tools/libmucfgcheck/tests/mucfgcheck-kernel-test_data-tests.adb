...
--  begin read only
   procedure Test_VTd_IRT_Region_Mapping (Gnattest_T : in out Test);
...
--  end read only
   ...
   begin
      ...
      VTd_IRT_Region_Mapping (XML_Data => Data);

      --  Kernel mapping by non-BSP CPU.
      ...
      exception
         when E : Validation_Error =>
            Assert (Condition => Ada.Exceptions.Exception_Message (X => E)
                    = "VT-d IR memory region 'vtd_ir' mapped by kernel running"
                    & " on CPU 1 instead of 0",
                    Message   => "Exception mismatch (1)");
      end;

      --  Multiple kernel mappings.
      ...
      exception
         when E : Validation_Error =>
            Assert (Condition => Ada.Exceptions.Exception_Message (X => E)
                    = "VT-d IR memory region 'vtd_ir' has multiple kernel "
                    & "mappings: 2",
                    Message   => "Exception mismatch (2)");
      end;
      ...
      Muxml.Utils.Set_Attribute
        (Doc   => Data.Doc,
         XPath => "/system/kernel/memory/cpu/memory[@logical='vtd_ir']",
         Name  => "physical",
         Value => "nonexistent");
      begin
         VTd_IRT_Region_Mapping (XML_Data => Data);
      ...
      exception
         when E : Validation_Error =>
            Assert (Condition => Ada.Exceptions.Exception_Message (X => E)
                    = "VT-d IR memory region 'vtd_ir' is not mapped by kernel",
                    Message   => "Exception mismatch (3)");
      end;
--  begin read only
   end Test_VTd_IRT_Region_Mapping;
--  end read only
