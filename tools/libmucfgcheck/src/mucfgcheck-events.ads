...
is
   --D @Section Id => validation-evts, Label => Events, Parent => validation, Priority => 0
   --D @Text Section => validation-evts, Priority => 0
   --D The following checks are performed to guarantee that events are
   --D correctly configured in the system policy.
   --D @UL Id => validators_evts, Section => validation-evts, Priority => 0

   --D @Item List => validators_evts, Priority => 0
