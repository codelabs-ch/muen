...
is
   ...
   --D @Section Id => validation-mem, Label => Memory, Parent => validation, Priority => 0
   --D @Text Section => validation-mem, Priority => 0
   --D The following checks are performed to verify that the memory is
   --D correctly configured in the system policy.
   --D @UL Id => validators_mem, Section => validation-mem, Priority => 0
