...
is
   ...
   --D @Section Id => validation-subj, Label => Subjects, Parent => validation, Priority => 0
   --D @Text Section => validation-subj, Priority => 0
   --D The following checks are performed to verify the correctness of the
   --D subject configuration in the system policy.
   --D @UL Id => validators_subj, Section => validation-subj, Priority => 0
