...
This script can be used to extend the muen toolchain. It transfers <doc ...> nodes
(for documentation) from the joined policy to their respective places in the policy-b
and saves the new policy-b as output.
...
class Graph:
    ...
    def __init__(self):
        # nodes:
        self.nodes = set() ##set of tuples: (name, {l,c,s}) ("Library","Component","Subject")
...
def do_traversal (current_map):
    ...
    return lambda node, param_bind, c2s_map, expansion : do_traversal_func (current_map=current_map, node=node, param_bind=param_bind, c2s_map=c2s_map, expansion=expansion)
...
def do_traversal_func (current_map, node, param_bind, c2s_map, expansion):
    ...
    for child in node.findall("*"):
        for rule in current_map:
            (rule_matches, fresh_param_bindings, action) = apply_rule (child, rule, current_map[rule], param_bind)
            if rule_matches:
...
def apply_rule (node, key, value, param_bind):
    ...
    rule_matches ... indicates if the first level (until first '/') of the given key matches node
    fresh_param_bindings ... mapping which var-name has been bound to which value
    ...
    # a new, shortened rule is produced from the old rule in case the given rule involed children of node
    new_action = (do_traversal ({remaining_rule : value}) if remaining_rule != '' else value)

    node_name = local_rule.split("[")[0]
    if node.tag != node_name: return (False, dict(), None)
...
def add_to_output (target_path):
    ...
    return lambda node, param_bind, c2s_map, expansion : \
        add_to_output_func (target_path=target_path[1:], node=node, param_bind=param_bind, c2s_map=c2s_map, exp_subdict=expansion)
...
def get_list_of_substitutions (parametrized_substring, param_bind, c2s_map):
    """The function returns a list of subject names either referencing the given component
    (resolved via param_bind) or simply the resolution of $subject_name
    parametrized_substring must be either   $Component_To_Subjects($component_name)
    of the from   $subject_name .

    """
    if parametrized_substring.startswith("$Component_To_Subjects"):
        param_name = parametrized_substring.split('$Component_To_Subjects($')[1].split(')')[0]
...
def add_to_output_func (target_path, node, param_bind, c2s_map, exp_subdict):
    ...
    if "$" not in local_path:
        ...
        add_to_output_func (target_path=remaining_path, node=node, param_bind=param_bind, c2s_map=c2s_map, \
                           exp_subdict=exp_subdict['__children'][local_path])
        return

    parametrized_substring = local_path.split('"')[1]
    substitutions = get_list_of_substitutions (parametrized_substring, param_bind, c2s_map)
...
def compile_target_expansion (source_root):
    ...
    {'__children' :
      {'system' :
       {'__new_leaves' : [<Element doc at 0x7fc793f93b80>, <Element doc at 0x7fc793f93500>]
        '__children' :
        {'subjects' :
          {'__children' :
            {'subject[@name="time"]' : ...}
    ...
    do_traversal(system_map)(node=source_root, param_bind=parameter_binding, c2s_map=c2s_map, expansion=expansion)
...
def apply_target_expansion (subtree_root, sub_expansion):
    """Applies the modifications given by sub_expansion to the xml-subtree with rooted at subtree_root"""
...
subj_mem_map = {'doc' : add_to_output ('/system/subjects/subject[@name="$subject_name"]/memory/memory[@logical="$subj_mem_logical"]')}

subject_map = {'doc': add_to_output ('/system/subjects/subject[@name="$subject_name"]'),
...
comp_mem_map = {'doc': add_to_output ('/system/subjects/subject[@name="$Component_To_Subjects($component_name)"]/memory/memory[@logical="$comp_mem_logical"]')}

comp_map = {'doc': add_to_output ('/system/subjects/subject[@name="$Component_To_Subjects($component_name)"]'),
            'requires/memory/memory[@logical="$comp_mem_logical"]'        : do_traversal (comp_mem_map),
            'requires/memory/array/memory[@logical="$comp_mem_logical"]'  : do_traversal (comp_mem_map),
            'requires/channels/reader[@logical="$comp_mem_logical"]'      : do_traversal (comp_mem_map),
            'requires/channels/writer[@logical="$comp_mem_logical"]'      : do_traversal (comp_mem_map),
            'requires/channels/array/reader[@logical="$comp_mem_logical"]': do_traversal (comp_mem_map),
            'requires/channels/array/writer[@logical="$comp_mem_logical"]': do_traversal (comp_mem_map)}
...
system_map = {'doc'                                 : add_to_output ('/system'),
              'memory/memory[@name="$mem_name"]/doc': add_to_output ('/system/memory/memory[@name="$mem_name"]'),
...
def main():
    parser = argparse.ArgumentParser(description='Transfer <doc> nodes from policy-joined to policy-b')
    parser.add_argument('--pj', type=str, required=True, help='path to policy_joined.xml')
    parser.add_argument('--pb', type=str, required=True, help='path to policy_b.xml')
    parser.add_argument('--o', type=str, required=True, help='path to output file (policy_b with documentation)')
    ...
    if '__children' in expansion and 'system' in expansion['__children']:
        apply_target_expansion(subtree_root=pb_root, sub_expansion=expansion['__children']['system'])
