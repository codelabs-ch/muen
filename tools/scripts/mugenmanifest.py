...
nodes = sorted(nodes, key=lambda c: muutils.ada_hex_to_int(c.get("physicalAddress")))


with open(sys.argv[2], "w") as f:
    f.write(
        "[Name;PhysicalAddress;MemorySize;Offset;ContentSize;Usage;Type;" "Content]\n"
    )
    ...
    for node in nodes:
        ...
        if is_file:
            ...
            if not content_size:
                sys.exit(
                    "Error: No size attribute in file node, "
            ...
            usage = str(((content_size_nr * 100) + (size_nr - 1)) / size_nr) + "%"
