...
package body Foo.Hasher
is

   subtype SHA256_Message_Range is LSC.SHA256.Message_Index range
     0 .. LSC.SHA256.Message_Index (Foo.Data_Range'Last) / 64 - 1;
   ...
   begin
      ...
      Output      := Foo.Null_Message;
      Output.Size := 32;
      for I in LSC.SHA256.SHA256_Hash_Index loop
         Output.Data (Foo.Data_Range (I * 4 + 1)) := SK.Byte'Mod
           (LSC_Hash (I));
         Output.Data (Foo.Data_Range (I * 4 + 2)) := SK.Byte'Mod
           (Interfaces.Shift_Right (Value  => LSC_Hash (I),
                                    Amount => 8));
         Output.Data (Foo.Data_Range (I * 4 + 3)) := SK.Byte'Mod
           (Interfaces.Shift_Right (Value  => LSC_Hash (I),
                                    Amount => 16));
         Output.Data (Foo.Data_Range (I * 4 + 4)) := SK.Byte'Mod
           (Interfaces.Shift_Right (Value  => LSC_Hash (I),
                                    Amount => 24));
      end loop;
   end SHA256_Hash;
