...
is
   ...
   --  Then, A is removed from N and its log-entry gets the value of A as
   --  Origin_Of_Node.
   procedure Move_Origin_To_Log (Doc : DOM.Core.Document);

   --  For each element-node in Doc, generate the Node_Backtrace_Log-entry and
   --  insert a comment is Doc (right before the node) containing that
   --  information.
   ...
   --  Inherits backtrace-information from Ancestor and
   ...
   --  Remove all entries in Nodes_Backtrace_Log for Node and its descendants.
   ...
   --  If Old_Node has no entry, a warning is given and an empty entry is
   --  created.
   ...
   --  Go to ancestors of Node and use their backtrace-log to write complete
   --  backtrace information to the log-entry for Node.
...
private
   ...
   --      instantiated templates, evaluated conditionals and amend statements.
   ...
   --  Return log-information for the given node as is.
