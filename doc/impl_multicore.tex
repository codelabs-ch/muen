...
inter-processor interrupts to the halted AP processors. According to the MP
specification, in order to wakeup the APs, the INIT-SIPI-SIPI\footnote{SIPI is
short for Startup IPI} IPI sequence must be sent to their APICs. See also figure
\ref{fig:mp-overview} for an illustration of this process. The SIPI IPI contains
the physical address vector 0x0 of the trampoline code copied to low-memory by
the init code. The AP processors jump to this code after wakeup. The trampoline
