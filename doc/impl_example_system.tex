...
native, trusted services with a minimal TCB. The trusted services might even be
formally verified.
...
If the user hits one of the special keys F1 to F6, the VT subjects updates the
...
untrusted xv6 subject and an event is triggered to inform the virtual keyboard
...
that on each key press, an interrupt with vector 33 is delivered to CPU0.
...
called, which has been changed to read keyboard scancodes from a shared memory
page instead of doing port I/O. The copied scancode data is then used to drive
