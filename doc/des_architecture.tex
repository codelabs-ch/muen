...
\emph{Interrupt events} simply allow to inject an interrupt to a destination
...
\emph{bootstrap processor}\index{Boostrap processor}(BSP) is responsible for
system initialization, while the other processors, called
\emph{application processors}\index{Application processor} (APs), wait for an
