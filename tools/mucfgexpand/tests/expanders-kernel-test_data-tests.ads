--  This package has been generated automatically by GNATtest.
--  Do not edit any part of it, see GNATtest documentation for more details.

--  begin read only
with Gnattest_Generated;

package Expanders.Kernel.Test_Data.Tests is

   type Test is new GNATtest_Generated.GNATtest_Standard.Expanders.Kernel.Test_Data.Test
   with null record;

   procedure Test_Add_Section_Skeleton_797fa9 (Gnattest_T : in out Test);
   --  expanders-kernel.ads:25:4:Add_Section_Skeleton

   procedure Test_Add_Binary_Mappings_234898 (Gnattest_T : in out Test);
   --  expanders-kernel.ads:28:4:Add_Binary_Mappings

   procedure Test_Add_Subj_VMCS_Mappings_0bc5b2 (Gnattest_T : in out Test);
   --  expanders-kernel.ads:31:4:Add_Subj_VMCS_Mappings

   procedure Test_Add_Subj_State_Mappings_5c5e62 (Gnattest_T : in out Test);
   --  expanders-kernel.ads:34:4:Add_Subj_State_Mappings

   procedure Test_Add_Subj_Timed_Event_Mappings_a6660d (Gnattest_T : in out Test);
   --  expanders-kernel.ads:37:4:Add_Subj_Timed_Event_Mappings

   procedure Test_Add_Subj_Interrupts_Mappings_4c7af6 (Gnattest_T : in out Test);
   --  expanders-kernel.ads:40:4:Add_Subj_Interrupts_Mappings

   procedure Test_Add_Subj_Sinfo_Mappings_d16a6f (Gnattest_T : in out Test);
   --  expanders-kernel.ads:43:4:Add_Subj_Sinfo_Mappings

   procedure Test_Add_Subj_MSR_Store_Mappings_09dcb8 (Gnattest_T : in out Test);
   --  expanders-kernel.ads:46:4:Add_Subj_MSR_Store_Mappings

   procedure Test_Map_Tau0_Interface_1c6595 (Gnattest_T : in out Test);
   --  expanders-kernel.ads:49:4:Map_Tau0_Interface

   procedure Test_Add_Devices_52dbbf (Gnattest_T : in out Test);
   --  expanders-kernel.ads:52:4:Add_Devices

   procedure Test_Remove_Diagnostics_Device_b093e6 (Gnattest_T : in out Test);
   --  expanders-kernel.ads:55:4:Remove_Diagnostics_Device

end Expanders.Kernel.Test_Data.Tests;
--  end read only
