...
the Muen kernel implementation and section \ref{sec:performance} analyzes the
...
\cite{IntelSDM}, volume 3A, section 3.4.3 and volume 3C, section 24.4.1). The
kernel initializes the segment registers of a subject to the values shown by
table \ref{tab:vmcs-segment-regs}. For an explanation of the access rights
format see Intel SDM \cite{IntelSDM}, volume 3C, section 24.4.1.
...
memory region over a segment selector cannot bypass paging and results in a page
fault. VM subjects are constrained by the Intel EPT mechanism, where access to
disallowed memory results in an EPT violation trap.
...
hand, the bitmask value of \texttt{0xffffffff} disallows modification of any
bit in CR0 for native subjects, paging is already enabled by default and cannot
be disabled. The remaining bits are identical. Bit 0 indicates protected mode
and bit 2 disables the x87 FPU for subjects. Bit 4 is reserved and must be set
for both profiles. Bit 5 enables native FPU error reporting (see table 9-3 in
\cite{IntelSDM}, volume 3A section 9.2.2).
...
subject leads to a trap, the VM profile allows the modification of bit 4 (Page
Size Extensions, PSE\index{PSE}). This allows 4 MB pages in 32-bit paging mode
(used for example by the xv6 OS\index{OS} described in section
\ref{subsubsec:xv6}).
...
the boundaries set by the Intel EPT mechanism, see section \ref{subsubsec:ept}.
A value moved into the CR3 control register is treated as a guest-physical
address and the instruction does not cause a VM exit.
...
about the meaning of the bits in the control registers, see Intel SDM
\cite{IntelSDM}, section 2.5.
...
test suite which provides wide code coverage. The tools will be released under
the GNU General Public License\index{GPL}.
